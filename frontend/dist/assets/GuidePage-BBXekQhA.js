import{r as e}from"./rolldown-runtime-hePW80VL.js";import{Ut as t,Xt as n,d as r,en as i,hn as a,in as o,mt as s}from"./vendor-icons-DWnlozLO.js";import{a as c}from"./vendor-query-WN1RjFf0.js";import{a as l,r as u,s as d}from"./index-DTjQ75Jy.js";import{t as f}from"./PageContainer-CNfLfV2e.js";import{t as p}from"./PageHeader-Dp_dFlJ3.js";import{n as m,t as h}from"./shared-DhP5VnTd.js";var g=e(a(),1),_=[`Dashboard`,`ONU Management`,`Templates`,`Traffic`,`Infrastructure`,`System`,`Activity`],v=[{id:`dashboard`,category:`Dashboard`,page:`/dashboard`,title:`Panduan Dashboard`,description:`Monitor status OLT dan ONU secara real-time`,steps:[{title:`Statistik ONU`,content:`Kartu statistik menampilkan total ONU per status: **Online**, **Offline**, **DyingGasp**, **LOS** (Loss of Signal), dan **Total**.
...
**Replace ONU (Swap SN/MAC)**: Ganti perangkat ONU rusak dengan SN/MAC baru tanpa konfigurasi ulang. Sistem akan: backup config lama → delete ONU lama → register ONU baru → re-apply config. Vendor harus sama (ZTE→ZTE, FiberHome→FiberHome).`},{title:`Jalur FTTH (Trace Kabel ke Pelanggan)`,content:`Kartu **Jalur FTTH** menampilkan rute lengkap dari OLT sampai ke ONU ini: OLT → PON → OTB (core) → JC (splice, kalau ada) → ODC (core) → ODP (port) → pelanggan.
...
Kalau ada bagian rantai yang belum lengkap datanya (mis. ODP belum di-assign ke ODC/JC), muncul kotak merah menandai persis di titik mana data yang kurang.`}],tips:[`EPON ONUs memiliki keterbatasan CLI — beberapa section mungkin tidak tersedia`,`WiFi config untuk EPON diambil dari DB (tidak dari OLT running-config)`,`Save Config untuk menyimpan perubahan ke startup-config OLT`,`Kalau ONU belum di-assign ke ODP manapun, kartu Jalur FTTH akan bilang begitu — assign lewat halaman FTTH Infrastructure`]},{id:`provision-wizard`,category:`ONU Management`,page:`/dashboard/onus/provision`,title:`Panduan Provision ONU`,description:`Provision ONU baru dengan wizard interaktif`,steps:[{title:`Pilih OLT & PON Port`,content:`Pilih OLT dari dropdown, lalu pilih PON port (frame/slot/port) tempat ONU terhubung.
...
Klik area untuk drill-down ke detail infrastruktur.`},{title:`Tree Tab`,content:`Tampilan pohon (hierarki) dari seluruh rantai fiber: OLT/PON → (opsional lewat JC) → OTB/ODF → (opsional lewat JC) → ODC → (opsional lewat JC) → ODP → (opsional lewat JC) → port pelanggan. Setiap segmen bisa punya JC di tengahnya, termasuk sebelum OTB (feeder trunk) dan sebelum pelanggan (kabel drop).
...
Saat tambah/edit OTB, ODC, atau ODP, ada toggle **"Fed From"**: pilih apakah node ini disambung langsung (OTB dari PON, ODC dari OTB, ODP dari ODC), atau lewat titik sambungan **JC** (lihat langkah berikutnya).`},{title:`JC (Joint Closure / Titik Sambungan) Tab`,content:`JC adalah titik sambungan (closure) di sepanjang jalur fiber — dipakai kalau kabel di segmen manapun (OLT/PON→OTB, OTB→ODC, ODC→ODP, atau ODP→pelanggan) melewati titik splice di lapangan, bukan sambungan langsung.
...
Klik marker untuk detail dan highlight jalur koneksinya. Garis menampilkan koneksi fiber (OLT → [JC] → ODC → [JC] → ODP → ONU).`},{title:`Dampak Downstream (Trace Kabel Putus)`,content:`Di Tree view, tiap baris OTB/JC/ODC/ODP punya ikon **orang (Users)** — klik untuk lihat berapa pelanggan yang terdampak kalau titik itu putus/bermasalah, lengkap dengan daftar nama dan status online/offline mereka.
...
Untuk trace jalur satu pelanggan spesifik (bukan sebaliknya), buka halaman **View ONU** pelanggan tersebut — ada kartu **Jalur FTTH** yang menampilkan rute lengkap dari OLT sampai ke pelanggan itu.`}],tips:[`JC bersifat opsional — kalau jalur fiber memang langsung tanpa titik sambungan, tidak perlu dibuat JC sama sekali, cukup OTB → ODC → ODP seperti biasa`,`JC di segmen OLT→OTB dan ODP→pelanggan jarang dipakai (biasanya OLT-OTB satu lokasi, drop cable langsung) — tapi kalau memang ada sambungan fisik di situ, sekarang bisa dicatat juga, bukan cuma di OTB→ODC / ODC→ODP`,`Menghapus JC tidak menghapus OTB/ODC/ODP/port pelanggan/JC lain yang tersambung ke situ — mereka cuma "dilepas" (feed source-nya jadi kosong), supaya data infrastruktur riil tidak ikut hilang`,`Nomor core "in" pada splice pakai penomoran tube milik parent (OTB/ODC/JC sumbernya), nomor core "out" pakai penomoran tube milik JC itu sendiri`,`Dampak downstream dan Jalur FTTH sama-sama jalan lewat berapa pun hop JC di tengahnya, tidak terbatas satu tingkat saja`]},{id:`customization`,category:`System`,page:`/dashboard/customization`,title:`Panduan Customization`,description:`Kustomisasi tampilan: kolom, filter, RX colors`,steps:[{title:`Desktop Columns`,content:`Atur visibilitas dan urutan kolom di tabel All ONUs untuk tampilan desktop.
...
Ingress rule otomatis ditambahkan untuk route subdomain ke Flask app.`}],prerequisites:[`Cloudflare API Token diperlukan (set di environment variable)`,`Cloudflare Tunnel harus sudah ter-install di server`]}];function y(e){let t=e.toLowerCase().trim();return t?v.filter(e=>e.title.toLowerCase().includes(t)||e.description.toLowerCase().includes(t)||e.category.toLowerCase().includes(t)||e.steps.some(e=>e.title.toLowerCase().includes(t)||e.content.toLowerCase().includes(t))||e.tips?.some(e=>e.toLowerCase().includes(t))):v}var b=c();function x(e){return e.split(/(\*\*[^*]+\*\*|\n)/g).map((e,t)=>e===`
`?(0,b.jsx)(`br`,{},t):e.startsWith(`**`)&&e.endsWith(`**`)?(0,b.jsx)(`strong`,{className:`text-tx1 font-semibold`,children:e.slice(2,-2)},t):(0,b.jsx)(`span`,{children:e},t))}function S({guide:e,defaultOpen:a}){let[o,c]=(0,g.useState)(a??!1);return(0,b.jsxs)(d,{bodyClassName:`p-0`,children:[(0,b.jsxs)(`button`,{onClick:()=>c(!o),className:`w-full flex items-center gap-3 p-4 text-left hover:bg-glass/50 transition-colors`,children:[(0,b.jsx)(`div`,{className:`flex-shrink-0 w-8 h-8 rounded-lg bg-accent/15 text-accent flex items-center justify-center`,children:(0,b.jsx)(t,{size:16})}),(0,b.jsxs)(`div`,{className:`flex-1 min-w-0`,children:[(0,b.jsx)(`h3`,{className:`text-sm font-semibold text-tx1 truncate`,children:e.title}),(0,b.jsx)(`p`,{className:`text-xs text-tx3 truncate`,children:e.description})]}),o?(0,b.jsx)(i,{size:18,className:`text-tx3 flex-shrink-0`}):(0,b.jsx)(n,{size:18,className:`text-tx3 flex-shrink-0`})]}),o&&(0,b.jsxs)(`div`,{className:`px-4 pb-4 space-y-3 animate-fade-in`,children:[e.prerequisites&&e.prerequisites.length>0&&(0,b.jsxs)(`div`,{className:`p-3 rounded-lg bg-warning/5 border border-warning/20 text-xs text-tx2`,children:[(0,b.jsxs)(`div`,{className:`flex items-center gap-1.5 mb-1.5 font-semibold text-warning`,children:[(0,b.jsx)(r,{size:14}),` Prasyarat`]}),(0,b.jsx)(`ul`,{className:`ml-4 space-y-1 list-disc`,children:e.prerequisites.map((e,t)=>(0,b.jsx)(`li`,{children:x(e)},t))})]}),(0,b.jsx)(`div`,{className:`space-y-2.5`,children:e.steps.map((e,t)=>(0,b.jsxs)(`div`,{className:`flex gap-2.5`,children:[(0,b.jsx)(`span`,{className:`flex-shrink-0 w-6 h-6 rounded-full bg-accent/15 text-accent flex items-center justify-center text-xs font-bold`,children:t+1}),(0,b.jsxs)(`div`,{className:`min-w-0 flex-1`,children:[(0,b.jsx)(`strong`,{className:`text-sm text-tx1`,children:e.title}),(0,b.jsx)(`p`,{className:`text-xs text-tx2 mt-0.5 leading-relaxed`,children:x(e.content)})]})]},t))}),e.tips&&e.tips.length>0&&(0,b.jsxs)(`div`,{className:`p-3 rounded-lg bg-glass border border-brd text-xs text-tx3`,children:[(0,b.jsxs)(`div`,{className:`flex items-center gap-1.5 mb-1.5 font-semibold text-tx2`,children:[(0,b.jsx)(s,{size:14,className:`text-accent`}),` Tips`]}),(0,b.jsx)(`ul`,{className:`ml-4 space-y-1 list-disc`,children:e.tips.map((e,t)=>(0,b.jsx)(`li`,{children:x(e)},t))})]})]})]})}function C(){let[e,n]=(0,g.useState)(``),[r,i]=(0,g.useState)(`All`),a=(0,g.useMemo)(()=>{let t=e?y(e):v;return r!==`All`&&(t=t.filter(e=>e.category===r)),t},[e,r]),s=(0,g.useMemo)(()=>{let e=new Map;for(let t of a)e.has(t.category)||e.set(t.category,[]),e.get(t.category).push(t);return Array.from(e.entries()).sort((e,t)=>{let n=_.indexOf(e[0]),r=_.indexOf(t[0]);return(n===-1?99:n)-(r===-1?99:r)})},[a]),c=(0,g.useMemo)(()=>{let e=new Map;for(let t of v)e.set(t.category,(e.get(t.category)||0)+1);return Array.from(e.entries()).sort((e,t)=>{let n=_.indexOf(e[0]),r=_.indexOf(t[0]);return(n===-1?99:n)-(r===-1?99:r)})},[]);return(0,b.jsxs)(f,{children:[(0,b.jsx)(p,{icon:(0,b.jsx)(o,{size:22,className:`text-accent`}),title:`Panduan`,description:`Pusat panduan penggunaan Salfanet NMS — ${v.length} panduan tersedia`}),(0,b.jsx)(m,{value:e,onChange:n,placeholder:`Cari panduan... (mis: ONU, VLAN, traffic, alert)`}),(0,b.jsxs)(h,{className:`gap-1.5`,children:[(0,b.jsxs)(`button`,{onClick:()=>i(`All`),className:`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${r===`All`?`bg-accent/15 text-accent border border-accent/20`:`bg-glass border border-brd text-tx2 hover:text-tx1 hover:border-accent/30`}`,children:[`Semua (`,v.length,`)`]}),c.map(([e,t])=>(0,b.jsxs)(`button`,{onClick:()=>i(e),className:`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${r===e?`bg-accent/15 text-accent border border-accent/20`:`bg-glass border border-brd text-tx2 hover:text-tx1 hover:border-accent/30`}`,children:[e,` (`,t,`)`]},e))]}),a.length===0?(0,b.jsx)(l,{icon:t,title:`Tidak ada panduan yang cocok dengan "${e}"`,action:(0,b.jsx)(u,{variant:`ghost`,onClick:()=>{n(``),i(`All`)},children:`Reset filter`})}):(0,b.jsx)(`div`,{className:`space-y-4`,children:s.map(([e,t])=>(0,b.jsxs)(`div`,{children:[(0,b.jsx)(`h2`,{className:`text-xs font-semibold text-tx3 uppercase tracking-wider mb-2 px-1`,children:e}),(0,b.jsx)(`div`,{className:`space-y-2`,children:t.map(e=>(0,b.jsx)(S,{guide:e,defaultOpen:a.length===1},e.id))})]},e))})]})}export{C as GuidePage};